...
def _import_media_and_labels_inputs(ctx, inputs):
    ...
    inputs.view("docs", types.Notice(label=f"Importer documentation: {docs_link}"))
...
def _import_labels_only(ctx):
    ...
    with contextlib.ExitStack() as exit_context:
        ...
        if labels_path is not None:
            data_path = {os.path.basename(p): p for p in ctx.dataset.values("filepath")}
...
def _get_merge_parameters(ctx, inputs):
    ...
    if ctx.params.get("include_info", None):
        inputs.bool(
            ...
            description=("Whether to overwrite existing dataset-level information"),
            view=types.CheckboxView(),
        )
...
def _export_samples_inputs(ctx, inputs):
    ...
    if export_type == "FILEPATHS_ONLY":
        ...
        dataset_types = _get_export_types(target_view, export_type, allow_coercion=True)
        inputs.enum(
            "dataset_type",
            dataset_types,
            required=True,
        ...
        inputs.view("docs", types.Notice(label=f"Exporter documentation: {docs_link}"))
    ...
    if _can_export_abs_paths(dataset_type):
        inputs.bool(
            ...
            description=("Store absolute paths to the media in the exported labels?"),
            view=types.CheckboxView(),
        )
...
def _get_export_types(view, export_type, allow_coercion=False):
    ...
    label_types = set(k for k, v in _LABEL_TYPES_MAP.items() if v in label_types)
